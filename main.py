...
if __name__ == '__main__':
    ...
    # Overwrite settings if "simulate" is requested (python main.py simulate)
    ...
    if TESTS["generate"]:
        user_request = input("🤖 Enter here your request :: ")
        knowledge_base.user_message = "%s [ %s ] %s" %(knowledge_base.user_message.split("[")[0],
                user_request, knowledge_base.user_message.split("]")[1])
    ...
    if not succesfull_validation and (TESTS["generate"] or TESTS["validate"]):
        while not succesfull_validation:
            ...
            if TESTS["generate"]:
                ...
                print (knowledge_base.user_message)
    ...
    ## Simulate system for final validation in iTHOR
    if TESTS["simulate"]:
